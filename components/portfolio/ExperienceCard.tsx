          ...
          return (
            <div
              ...
              className={`relative pl-6 border-l-2 transition-all ${isHighlighted
                  ? 'border-indigo-500'
                  : 'border-zinc-700/50 hover:border-indigo-500/50'
                }`}
            >
              <div className="absolute -left-[9px] top-0 h-4 w-4 rounded-full border-2 border-zinc-100 bg-zinc-100 opacity-0 transition-opacity group-hover:opacity-100" />
              ...
              <div className="mb-3">
                ...
                <div className="flex flex-wrap items-center gap-2 text-sm text-zinc-400">
                  <span className="font-medium text-indigo-400">{role.company}</span>
                  <span>•</span>
              ...
              {role.technologies && role.technologies.length > 0 && (
                <div className="mt-4 flex flex-wrap gap-2">
                  {role.technologies.map((tech, idx) => (
                    <span
                      key={idx}
                      className="rounded-full bg-indigo-500/10 px-3 py-1 text-xs font-medium text-indigo-300 border border-indigo-500/10"
                    >
                      {tech}
                    </span>
                  ))}
