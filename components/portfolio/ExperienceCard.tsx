          ...
          return (
            <div
              ...
              className={`relative pl-6 border-l-2 transition-all ${
                isHighlighted
                  ? "border-amber-500"
                  : "border-amber-200/50 hover:border-amber-400/50"
              }`}
            >
              <div className="absolute -left-[9px] top-0 h-4 w-4 rounded-full border-2 border-amber-200 bg-amber-50 opacity-0 transition-opacity group-hover:opacity-100" />
                ...
                <ul className="mt-4 space-y-2 text-sm text-stone-600">
                  {role.achievements.map((achievement, idx) => (
                    <li key={idx} className="flex items-start">
                      <span className="mr-2 mt-1.5 h-1.5 w-1.5 flex-shrink-0 rounded-full bg-amber-500/50" />
              ...
              {role.technologies && role.technologies.length > 0 && (
                <div className="mt-4 flex flex-wrap gap-2">
                  {role.technologies.map((tech, idx) => (
                    <span
                      key={idx}
                      className="rounded-full bg-amber-500/10 px-3 py-1 text-xs font-medium text-amber-700 border border-amber-500/20"
                    >
                      {tech}
                    </span>
                  ))}
