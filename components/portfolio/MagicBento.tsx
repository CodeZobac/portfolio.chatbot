...
export const createParticleElement = (
  ...
  el.style.cssText = `
    ...
    height: 7px;
    border-radius: 50%;
    background: radial-gradient(circle, rgba(${color}, 1) 0%, rgba(${color}, 0.7) 65%, rgba(${color}, 0) 100%);
    box-shadow: 0 0 14px rgba(${color}, 0.95), 0 0 28px rgba(${color}, 0.55);
    pointer-events: none;
    z-index: 100;
    left: ${x}px;
    top: ${y}px;
...
export const ParticleCard: React.FC<{
  ...
  const animateParticles = useCallback(() => {
    if (isPaused || !cardRef.current || !isHoveredRef.current) return;
    ...
    memoizedParticles.current.forEach((particle, index) => {
      const timeoutId = setTimeout(() => {
        if (isPaused || !isHoveredRef.current || !cardRef.current) return;
        ...
        gsap.fromTo(
          clone,
          { scale: 0, opacity: 0 },
          { scale: 1.15, opacity: 1, duration: 0.25, ease: "back.out(2)" },
        );
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      if (isPaused || (!enableTilt && !enableMagnetism)) return;
    ...
    const handleClick = (e: MouseEvent) => {
      if (onClick) onClick();
      if (!clickEffect || isPaused) return;
      ...
      <style>
        {`
          .bento-section {
            ...
            --background-dark: rgba(255, 250, 244, 0.96);
            --white: #1c1917;
            --amber-primary: rgba(232, 168, 56, 1);
            --amber-glow: rgba(232, 168, 56, 0.15);
            --amber-border: rgba(232, 168, 56, 0.6);
          }
          ...
          .card--border-glow:hover {
            box-shadow: 0 10px 30px rgba(232, 168, 56, 0.18), 0 0 40px rgba(${glowColor}, 0.12);
          }
          ...
          .particle-container:hover {
            box-shadow: 0 4px 20px rgba(232, 168, 56, 0.2), 0 0 30px rgba(${glowColor}, 0.2);
          }
      ...
      <BentoCardGrid gridRef={gridRef}>
        <div className="card-responsive grid gap-4">
          {projects.map((project, index) => {
            ...
            const isZooming = zoomingCardId === project.id;
            ...
            if (enableStars) {
              return (
                ...
                >
                  ...
                  {/* Middle - Lens wrapped Image */}
                  <div className="relative w-full px-3 mb-1 flex-grow flex items-center justify-center">
                    <div className="relative w-full aspect-[16/9] rounded-xl overflow-hidden shadow-inner border border-stone-200/60 bg-stone-50 pointer-events-auto">
                      <Lens
                        zoomFactor={1.7}
                        lensSize={200}
                        onHoverChange={(hovering) =>
                          handleZoomChange(hovering ? project.id : null)
                        }
                        onClick={handleCardClick}
                      >
                        <Image
                  ...
                  {/* Footer - Tech Stack */}
                  <div className="mt-auto flex flex-nowrap overflow-hidden gap-1.5 px-3 pb-3 pt-1 flex-shrink-0 z-10 relative pointer-events-none">
                    {project.techStack.slice(0, 3).map((tech, i) => (
                      <span
                        key={i}
                        className="text-[9px] px-1.5 py-0.5 rounded-md bg-stone-100 text-stone-700 font-semibold border border-stone-200 whitespace-nowrap"
                      >
                        {tech}
                      </span>
                    ))}
            ...
            return (
              <div
                ...
                onClick={handleCardClick}
                ref={(el) => {
                  if (!el || isZooming) return;

                  const handleMouseMove = (e: MouseEvent) => {
                    if (shouldDisableAnimations || isZooming) return;
                  ...
                  const handleClick = (e: MouseEvent) => {
                    if (!clickEffect || shouldDisableAnimations || isZooming)
                      return;
